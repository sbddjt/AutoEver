...
- **독자적인 기능을 갖는 구성 요소:** 함수, 클래스, 파일 등을 통칭합니다.
- **파이썬에서의 정의:** 하나의 **파이썬 파일(`.py`)**을 곧 모듈이라고 합니다.

### 🏗 종류

1. **표준 모듈:** 파이썬 설치 시 기본적으로 포함된 모듈 (예: `math`, `sys`, `os`).
2. **사용자 정의 모듈:** 개발자가 직접 작성한 `.py` 파일.
3. **3rd Party Module:** 외부 개발자/단체에서 만든 모듈. 공식 저장소에서 `pip` 명령어로 다운로드하여 사용합니다.
...
| **사용 형식** | **설명** |
| --- | --- |
| `import 모듈이름` | 모듈 전체를 가져옵니다. 사용할 때 `모듈이름.함수()` 형식을 유지해야 합니다. |
| `from 모듈이름 import 속성/함수` | 특정 요소만 가져옵니다. 모듈 이름을 생략하고 직접 호출할 수 있습니다. |
| `from 모듈이름 import *` | 모듈의 모든 요소를 가져오며 이름을 생략할 수 있지만, 출처가 불분명해질 수 있습니다. |
| `import 모듈이름 as 새로운이름` | 모듈에 별명이 아닌 **새로운 이름**을 부여합니다. 기존 이름은 사용할 수 없습니다. |
| `from ... import ... as ...` | 특정 함수나 속성에만 새로운 이름을 부여하여 사용합니다. |

> 💡 메모리 효율 관점의 차이
> 
> - `import math`: `math.py`의 모든 내용과 `PI` 등을 메모리에 올립니다.
> - `from math import PI`: 필요한 `PI`만 메모리에 올려 실행하므로 간결합니다.

---

### 🔍 모듈 탐색 위치 및 순서 (`sys.path`)

파이썬은 모듈을 찾을 때 다음 순서대로 검색합니다:

1. **현재 디렉토리** (프로그램이 실행되는 위치)
2. **파이썬 설치 디렉토리** (표준 라이브러리 경로)
...
- **프로그램 내부 추가:** `sys.path.append("경로")`를 사용하며 해당 프로그램 실행 중에만 유효합니다.
- **운영체제 환경 변수(`PYTHONPATH`) 설정:** 모든 프로그램에 적용됩니다.
    - **Windows:** `set PYTHONPATH = 경로`
    - **MAC / Linux (Bash):** `export PYTHONPATH = 경로` **(이건 외우자)**
    - **Linux (C Shell):** `setenv PYTHONPATH 검색경로`
...
- **관련 있는 모듈들의 모임:** 모듈들을 폴더 단위로 관리하는 개념입니다.

### 🛠 패키지 생성 및 사용

1. **직접 생성:** 모듈 파일들을 하나의 디렉토리에 모으고, 내부에 `__init__.py` 파일을 생성하면 해당 디렉토리는 패키지로 인식됩니다.
2. **가져오기:** `from 패키지이름 import 모듈이름` 형식을 사용합니다.

### 📂 site-packages 디렉토리

- 외부에서 설치한(3rd party) 패키지들이 저장되는 기본 위치입니다.
- 나중에 **도커 이미지(Image)** 등을 만들 때 이 디렉토리의 구성 요소를 파악하는 것이 중요합니다.
